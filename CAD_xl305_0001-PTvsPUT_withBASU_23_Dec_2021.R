...
dat.dir     <- "/Users/xz289/Documents/Xuan_Li/xl315_0001"
setwd(dat.dir)
...
normBatchBox
dev.off()
## customised a good pca plot for paper. revised 26/01/2022 by changing the groups names
## to activated and primed.
...
## PT PUT PCA
customised_PCA1 <- function (MSnSetObj,transform = TRUE, colourBy = "SampleGroup", title = "", labelColumn = "BioRep", 
                            labelsize = 4, pointsize = 3, x.nudge = 4, x.PC = 1) 
{
  intensities <- exprs(MSnSetObj) %>% as.data.frame() %>% na.omit()
  if (!nrow(intensities)) {
  ...
  intensities1  <- intensities[, grep("PT|PUT", colnames(intensities))]
  pca <- intensities %>% t() %>% prcomp()
  pcaVariance <- round((pca$sdev^2/sum(pca$sdev^2)) * 100)
  ...
  xPC <- str_c("PC", x.PC) %>% sym()
  yPC <- str_c("PC", x.PC + 1) %>% sym()
  plotDat.sub <- plotDat[grep("PT|PUT", plotDat$SampleGroup),]
  pcPlot <- ggplot(plotDat.sub, aes(x = PC1, y = PC2, colour=SampleGroup,  shape=RepName)) +
    geom_point(size = pointsize) + 
    scale_color_manual(name="Group", values = c( "red2", "tan2")) + 
    labs(x = xLab, y = yLab, fill = NULL, title = title) + 
    scale_shape_manual(name="Batch",values=c(0,1,2)) + 
    geom_hline(yintercept=0, linetype="dashed", color = "black", size=0.2) +
    geom_vline(xintercept=0, linetype="dashed", color = "black", size=0.2) +
    #geom_text_repel(aes(label=SampleName), show.legend = FALSE, size=2) +
    theme_bw() + theme(text = element_text(size = 14), plot.title = element_text(size = 14, hjust = 0.5), aspect.ratio = 1) +
    theme( panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
...
## Add PT and PUT PCA 
pdf("PTvsPUT_reviewed_withBP_N1252_rmBatch_customised_PCA_POnly_23_Dec_2021.pdf")
PCA_cust1 <- customised_PCA1(MSnSetObj,transform = TRUE, colourBy = "SampleGroup", title = "", labelColumn = "BioRep", 
                           labelsize = 4, pointsize = 4, x.nudge = 4, x.PC = 1) 
print(PCA_cust1)
dev.off()
...
pdf("PTvsPUT_reviewed_withBP_N1252_rmBatch_hclust_23_Dec_2021.pdf")
hierarchicalPlot(MSnSet_normR_batch)
dev.off()

pdf("PTvsPUT_reviewed_withBP_N1252_corrPlot_23_Dec_2021.pdf")
corrPlot(MSnSet_normR_batch,addValues=FALSE)
dev.off()
...
## Wait for Xuan's check came back 
...
message("+--- choose Microtubule pathways dot plot with padj values --------+")

pdf("Protein_190920_N7216_F1252_reviewed_withBP_Microtubule_sel_dotplot_17_Jan_2022.pdf", width=6, height=4)
selected_pathways <- GO.all$Description[GO.all$ID%in%G7.subdat$ID==T]
dotplot(GO.all, showCategory=selected_pathways)
dev.off()

write.xlsx(newPaths.all, file = "Selected_allGOs_summary_Jan_2022.xlsx")
...
message("+--- Possible removing the rest of code with #### -----+")
############################################################################################################
GreenList  <- c("Aldob", "Birc6", "Cdk1", "Cdk2", "Chd4", "Ecpas", "Gnai1", "Gnai2", "Hnrnpu",
               "Hspa1a", "Hspa1b", "Krt18", "Mark2", "Nek6", "Nubp2", "Pdcd6ip", "Rbbp6",
               "Rps7")
RedList    <- c("Arhgef7", "Hspa1a", "Hspa1b")
OrangeList <- c("Capzb", "Cct7", "Cltc", "Dync1h1", "Ezr", "Iqgap1", "Iqgap2", "Kif23",
                "Kif3a", "Kif3b", "Lmna", "Nudc", "Ovgp1", "Rbm39", "Rps3", "Shmt2", "Smn1",
                "Son", "Tuba1a", "Tuba1c", "Tuba3a", "Tuba4a", "Tuba8", "Tubb4b", "Vps4b")
## Arhgef7 padj > 0.05 and Capzb, Vps4b not being picked for the name_1006 did not relating to microtubule.
GreenT <- "microtubule organizing center" ## add two new genes Plk1 and Eif3a, removed Mark2, 19 genes
RedT   <- "microtubule nucleation" ## only 2 with positive regulation, rm Arhgef7 due to padj > 0.05

subGreen <- subset(PTvsPUT_oriR.microtubu, name_1006==GreenT & adj.P.Val <=0.05)
subRed   <- PTvsPUT_oriR.microtubu[grep("microtubule nucleation",PTvsPUT_oriR.microtubu$name_1006),]
subRed   <- subset(subRed, adj.P.Val <=0.05)
subOrg   <- PTvsPUT_oriR.microtubu[PTvsPUT_oriR.microtubu$GeneSymbol%in%OrangeList,]
subOrg   <- subset(subOrg, adj.P.Val <=0.05) ## rm Vps4b and Capzb


## heatmap for each green color 
subGreen  <- subGreen[order(-subGreen$log2FC),]
heatmat.GreenM <- cbind(rowMeans(subGreen[,8:10]), rowMeans(subGreen[,11:13]))
colnames(heatmat.GreenM)  <- c("PT", "PUT")
rownames(heatmat.GreenM)  <- subGreen$GeneSymbol
o2matGreen     <- seriate(dist(t(heatmat.GreenM)), method = "GW")
L2FC.Green     <- as.matrix(subGreen[,"log2FC"], ncol=1)
colnames(L2FC.Green) <- "L2FC"
rownames(L2FC.Green) <- subGreen[,"GeneSymbol"]


pht_heatGreen = Heatmap(as.matrix(heatmat.GreenM), col = heat.colors(1000),  
                       name = "Microtubule organizing center", show_row_names=T,
                       row_names_side="left",
                       show_column_names = T, width = unit(4, "cm"),
                       heatmap_legend_param =list(title = "Log2(Norm.Intens+1)",
                                                  title_position = "leftcenter-rot",
                                                  title_gp = gpar(fontsize = 10, fontface="bold"),
                                                  labels_gp = gpar(fontsize = 10),
                                                  legend_width = unit(0.8,"cm"),
                                                  legend_height= unit(3, "cm")),
                       row_title = "Microtubule organizing center",
                       cluster_rows = F, 
                       show_column_dend = F,
                       column_title=NULL,
                       row_title_rot = 90,
                       ...
                       cluster_columns = reorder(as.dendrogram(o2matM[[1]]), c(2,1)), 
                       column_names_gp = gpar(fontsize = 10, col = rep(c("red","blue"), c(1,1))))+
  Heatmap(L2FC.Green, name = "l2FC", col=ScaleCols.l2, width = unit(5, "mm"),
          heatmap_legend_param = list(title = "Log2FC",title_position = "leftcenter-rot",
                                      legend_width = unit(0.8,"cm"),
                                      legend_height= unit(4, "cm")), 
          show_row_names=F,row_names_gp = gpar(fontsize = 10),
          column_names_gp = gpar(fontsize = 9,  fontface="bold"),
...
pdf("PTvsPUT_microtubule_selMarkers_organizingcenter_heatmap_23_Dec_2021.pdf")
print(pht_heatGreen)
dev.off()
## A big heatmap with the above selected genes and pathways.
selPaths  <- unique(c(subGreen$name_1006, subRed$name_1006,subOrg$name_1006)) ## 22 of them
selDat    <- rbind(subGreen, subRed, subOrg)[, c("GeneSymbol", "name_1006", "log2FC")]
heatmatS.rowN <- selPaths
heatmatS.colN <- unique(selDat$GeneSymbol)
heatmatS  <- matrix(0, nrow=length(heatmatS.rowN), ncol= length(heatmatS.colN))
rownames(heatmatS) <- heatmatS.rowN
colnames(heatmatS) <- heatmatS.colN

for( i in 1:length(heatmatS.rowN)){
  subsel <- subset(selDat, name_1006==rownames(heatmatS)[i])
  print(i)
  jind   <- lapply(colnames(heatmatS), function(x) which(x==subsel$GeneSymbol))
  for(j in 1:length(heatmatS.colN)){
    if(length(jind[[j]])==0){
      heatmatS[i,j] <- 0
    }else{
      heatmatS[i,j] <- subsel[subsel$GeneSymbol==heatmatS.colN[j],"log2FC"]
    }
  }
  heatmatS
}

newheatmatS <- heatmatS[-1,]  
newheatmatS <- newheatmatS[,-which(colSums(newheatmatS)==0)]
otherMicrot <- Heatmap(newheatmatS, name = "log2FC", show_row_names=T,
                row_names_side="left", cluster_rows=T, show_row_dend = F, 
                cluster_columns = T, show_column_dend = F, row_names_gp = gpar(fontsize = 9),
                column_names_gp = gpar(fontsize = 9),
                width = ncol(newheatmatS)*unit(5, "mm"), 
                height = nrow(newheatmatS)*unit(5, "mm"))
pdf("PTvsPUT_microtubule_selMarkers_others_heatmap_23_Dec_2021.pdf", width=11, height=6)
print(otherMicrot)
dev.off()
####################################################################################################

message("+Kegg pathway Covid genes cnetwork with log2FoldChange only------+ ")

pdf("PTvsPUT_Kegg_cov19_cnetplot_Jan_2022.pdf")
cnetplot(GO.KEGG.all, showCategory = "Coronavirus disease - COVID-19", foldChange= PgeneL.new)
dev.off()
...
message("+---- tsne plot for PT and PUT ----------------+")

test1 <- which(duplicated(PTvsPUT_locs[,10:12])==T)
test2 <- which(duplicated(PTvsPUT_locs[,13:15])==T)
test  <- unique(c(test1,test2))
PTvsPUT_locs  <- PTvsPUT_locs[-test,]
#Main.location <- unlist(lapply(PTvsPUT_locs$Main.location, function(x) strsplit(x, split=";")[[1]][1]))
#PTvsPUT_locs$Main.location <- Main.location
message("+--- Perform PUT clustering--------------------+")

PUT_locs  <- PTvsPUT_locs[,-c(10:12)]
train     <- PUT_locs %>% drop_na() %>% 
  dplyr::select(-c(Gene.name, log2FC, adj.P.Val, t, B))%>%
  mutate(ID=row_number()) 

train_meta <- train %>%
  dplyr::select(ID,Main.location,Reliability,GeneSymbol)

set.seed(142)
train_tSNEfit <- train %>% dplyr::select(where(is.numeric)) %>%
  column_to_rownames("ID") %>% scale() %>% Rtsne()
train_tSNEdf <- train_tSNEfit$Y %>% as.data.frame() %>%
  rename(tSNE1="V1",tSNE2="V2") %>%
  mutate(ID=row_number())
train_tSNEdf <- train_tSNEdf %>% inner_join(train_meta, by="ID")
train_tSNEdf %>% head()
## KNN graph and Louvain community detection 
library("igraph")
library("FNN")
k = 100
train_knnnorm = get.knn(as.matrix(train_tSNEfit$Y), k = k)
train_knnnorm = data.frame(from = rep(1:nrow(train_knnnorm$nn.index), k), 
                           to = as.vector(train_knnnorm$nn.index), 
                           weight = 1/(1 + as.vector(train_knnnorm$nn.dist)))
train_nwnorm = graph_from_data_frame(train_knnnorm, directed = FALSE)
train_nwnorm = simplify(train_nwnorm)
train_lcnorm = cluster_louvain(train_nwnorm)
train_tSNEdf$louvain = as.factor(membership(train_lcnorm))
train_lccent = train_tSNEdf %>% group_by(louvain) %>% 
  dplyr::select(tSNE1, tSNE2) %>% summarize_all(mean)
PUT_tsne <- ggplot(train_tSNEdf, aes(x = tSNE1, y = tSNE2, colour = louvain)) + 
  geom_point(alpha = 0.3) + theme_bw() + 
  geom_label_repel(aes(label = louvain), data = train_lccent) +
  guides(color = guide_legend(reverse = F))


message("+--- Perform PT clustering--------------------+")
PT_locs   <- PTvsPUT_locs[,-c(13:15)]

test      <- PT_locs %>% drop_na() %>% 
  dplyr::select(-c(Gene.name, log2FC, adj.P.Val, t, B))%>%
  mutate(ID=row_number()) 

test_meta <- test %>%dplyr::select(ID,Main.location,Reliability,GeneSymbol)

set.seed(142)
test_tSNEfit <- test %>% dplyr::select(where(is.numeric)) %>%
  column_to_rownames("ID") %>% scale() %>% Rtsne()
test_tSNEdf <- test_tSNEfit$Y %>% as.data.frame() %>%
  rename(tSNE1="V1",tSNE2="V2") %>%
  mutate(ID=row_number())
test_tSNEdf <- test_tSNEdf %>% inner_join(test_meta, by="ID")
test_tSNEdf %>% head()
## KNN graph and Louvain community detection 

test_knnnorm = get.knn(as.matrix(test_tSNEfit$Y), k = k)
test_knnnorm = data.frame(from = rep(1:nrow(test_knnnorm$nn.index), k), 
                          to = as.vector(test_knnnorm$nn.index), 
                          weight = 1/(1 + as.vector(test_knnnorm$nn.dist)))
test_nwnorm = graph_from_data_frame(test_knnnorm, directed = FALSE)
test_nwnorm = simplify(test_nwnorm)
test_lcnorm = cluster_louvain(test_nwnorm)
test_tSNEdf$louvain = as.factor(membership(test_lcnorm))
test_lccent = test_tSNEdf %>% group_by(louvain) %>% 
  dplyr::select(tSNE1, tSNE2) %>% summarize_all(mean)
PT_tsne <- ggplot(test_tSNEdf, aes(x = tSNE1, y = tSNE2, colour = louvain)) + 
  geom_point(alpha = 0.3) + theme_bw() + 
  geom_label_repel(aes(label = louvain), data = test_lccent) +
  guides(color = guide_legend(reverse = F))
pdf("PUT_and_PT_subcellular_Raw_Mean_tsne_localisation_23_Dec_2021.pdf", width=6, height=12)
plot_grid(PUT_tsne,PT_tsne, nrow=2)
dev.off()
message("+---check train PUT and test PT all genes corresponding group -----+")

train.names <- paste0(train_tSNEdf$GeneSymbol,"_", train_tSNEdf$Main.location,
                      "_", train_tSNEdf$louvain)
test.names  <- paste0(test_tSNEdf$GeneSymbol,"_", test_tSNEdf$Main.location,
                      "_", test_tSNEdf$louvain)

tcombine      <- cbind(train.names, test.names)
tcombine.sub1 <- subset(tcombine, train.names!=test.names)
tcombine.sub2 <- subset(tcombine, train.names==test.names)

tcom.genes    <- unlist(lapply(tcombine.sub2[,1], function(x) strsplit(x, split="_")[[1]][1]))
tcom.groups   <- unlist(lapply(tcombine.sub2[,1], function(x) strsplit(x, split="_")[[1]][3]))
table(tcom.groups)
## 1  2  3  4  5  6  7  8 
## 19 10  8 62  2 33
tcom.gList    <- list()
for(i in 1:8){
  tcom.gList[[i]] <- tcom.genes[tcom.groups==i]
  tcom.gList
}

newdata1 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[1]],]
newdata2 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[2]],]
newdata3 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[3]],]
newdata4 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[4]],]
newdata5 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[5]],]
newdata6 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[6]],]
newdata7 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[7]],]
newdata8 <- test_tSNEdf[test_tSNEdf$GeneSymbol%in%tcom.gList[[8]],]
HPT <- PT_tsne + geom_point(data=newdata1,colour="red", cex=1.2) +
  geom_point(data=newdata2,colour="brown", cex=1.2) +
  geom_point(data=newdata3,colour="green", cex=1.2) +
  geom_point(data=newdata4,colour="darkgreen", cex=1.2) +
  geom_point(data=newdata5,colour="blue", cex=1.2) +
  geom_point(data=newdata6,colour="darkblue", cex=1.2) +
  geom_point(data=newdata7,colour="purple", cex=1.2) +
  geom_point(data=newdata8,colour="deeppink", cex=1.2)+
  ggtitle("PT")+
  theme(plot.title = element_text(color="black", size=14, face="bold", hjust = 0.5))

newdata1U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[1]],]
newdata2U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[2]],]
newdata3U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[3]],]
newdata4U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[4]],]
newdata5U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[5]],]
newdata6U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[6]],]
newdata7U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[7]],]
newdata8U <- train_tSNEdf[train_tSNEdf$GeneSymbol%in%tcom.gList[[8]],]
HPUT <- PUT_tsne + geom_point(data=newdata1U,colour="red", cex=1.2) +
  geom_point(data=newdata2U,colour="brown", cex=1.2) +
  geom_point(data=newdata3U,colour="green", cex=1.2) +
  geom_point(data=newdata4U,colour="darkgreen", cex=1.2) +
  geom_point(data=newdata5U,colour="blue", cex=1.2) +
  geom_point(data=newdata6U,colour="darkblue", cex=1.2) +
  geom_point(data=newdata7U,colour="purple", cex=1.2) +
  geom_point(data=newdata8U,colour="deeppink", cex=1.2) +
  ggtitle("PUT")+
  theme(plot.title = element_text(color="black", size=14, face="bold", hjust = 0.5))

## need to think how to figure out the changes.
pdf("PUT_and_PT_subcellular_withBP_Raw_Mean_tsne_localisation_HighlightCommon_23_Dec_2021.pdf", width=6, height=12)
plot_grid(HPUT,HPT, nrow=2)
dev.off()
...
message("+---- boxplot with plk1 normalisation for PT and PUT, suppFig3(b)----+")

diffstats.plk1 <- computeDiffStats(MSnSet_normR_batch_nplk1, contrasts=contrasts)
diffexp.plk1 <- list()
for (i in 1:length(contrasts))
  diffexp.plk1[[i]] <- getContrastResults(diffstats=diffstats.plk1, contrast=contrasts[i], writeFile= TRUE)
for (i in 1:length(contrasts))
{
  toplot <- diffexp.plk1[[i]]$Accessions[diffexp.plk1[[i]]$adj.P.Val < 0.05]
  if(length(toplot) > 10)
    toplot <- toplot[1:10]
  print(maVolPlot(diffstats.plk1, contrast = contrasts[i], plotType="MA", title= contrasts[i], selectedGenes =
                    toplot, fdrCutOff=0.05))
}

i = 1
BTvsBUT_oriR.plk1 <- diffexp.plk1[[i]]
BTvsBUT_oriR.plk1 <- BTvsBUT_oriR.plk1[, c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val", "t", "B",
                                           "BT_1", "BT_2", "BT_3", "BUT_1", "BUT_2", "BUT_3", "Description")]
i = 2
PTvsPUT_oriR.plk1 <- diffexp.plk1[[i]]
PTvsPUT_oriR.plk1 <- PTvsPUT_oriR.plk1[, c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val", "t", "B",
                                           "PT_1", "PT_2", "PT_3", "PUT_1", "PUT_2", "PUT_3", "Description")]
i = 3
PTvsBT_oriR.plk1 <- diffexp.plk1[[i]]
PTvsBT_oriR.plk1 <- PTvsBT_oriR.plk1[, c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val", "t", "B",
                                         "PT_1", "PT_2", "PT_3", "BT_1", "BT_2", "BT_3", "Description")]
i = 4
PUTvsBUT_oriR.plk1 <- diffexp.plk1[[i]]
PUTvsBUT_oriR.plk1 <- PUTvsBUT_oriR.plk1[, c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val", "t", "B",
                                             "PUT_1", "PUT_2", "PUT_3", "BUT_1", "BUT_2", "BUT_3", "Description")]
selPlk1Markers <- c("Ddx3x", "Btk", "Syk", "Eif2ak2", "Mark2", "Gbp5", "Nlrp3")
oriR.plk1 <- PTvsPUT_oriR[PTvsPUT_oriR$GeneSymbol%in%selPlk1Markers,]
norR.plk1 <- PTvsPUT_oriR.plk1[PTvsPUT_oriR.plk1$GeneSymbol%in%selPlk1Markers,]

merPlk1    <- merge(oriR.plk1[,c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val")],
                    norR.plk1[,c("Accessions", "GeneSymbol", "log2FC", "adj.P.Val")], by = "GeneSymbol")[,-5]
colnames(merPlk1) <- c("GeneSymbol", "Accessions", "log2FC.ori", "Padj.ori", "log2FC.Plk1", "Padj.Plk1")
write.csv(merPlk1, file = "SelMarkers_PTvsPUT_oriR_normPlk1_L2FC_Padj_summary_Jan_2022.csv", row.names=F)

message("+---- ExtFig5e heatmap gene name conveted to protein name------------+")

library(biomaRt)
ensembl    =  useEnsembl(biomart="ENSEMBL_MART_ENSEMBL", dataset="mmusculus_gene_ensembl",host = 'ensembl.org')
ss <- listAttributes(ensembl)
ensUni     <- getBM(attributes=c('external_gene_name', 'uniprot_gn_symbol', 'uniprot_isoform', 'uniprotsptrembl'), 
